...
fn main() {
    ...
    match cli.command {
        ...
        Command::Update { experimental }                  => cmd_update(experimental),
...
fn cmd_update(experimental: bool) {
...
fn write_project_readme(project_root: &Path) {
...
fn delete_project_readme(project_root: &Path) {
    let path = project_root.join("README.md");
    if path.exists() {
...
// list every .bu source file in this folder (no extension)
soldier
helper
```
...
| `#include:` | Name of a sibling/child folder to import             |

Lines that are not directives are treated as source-file references (one per
line, without the `.bu` extension).

---

## Source files

A `.bu` source file contains function definitions:
...
// Using a stdlib builtin
let shout(s: String) -> result: String {
    builtin::to_upper
}

// Returning early
let safe_div(a: i32, b: i32) -> result: i32 {
    if b == 0 {
        result = 0
        return
    }
...
return   // exits the current function (result holds the return value)
```

### Escape blocks (raw target code)
...
Only the escape block matching the active backend is emitted; all others are
ignored.
...
| `bullang init <name> --blueprint file.bu` | Init from a blueprint file            |
...
| `bullang update`               | Pull & reinstall the latest version               |
...
# 4. Transpile — this file will be deleted automatically
